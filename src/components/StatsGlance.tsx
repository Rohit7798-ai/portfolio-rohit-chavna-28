...
const StatsGlance = () => {
  ...
  useEffect(() => {
    ...
    const checkTimer = setInterval(() => {
      if (counters.projects >= finalStats.projects && counters.models >= finalStats.models && counters.tools >= finalStats.tools && counters.feedback >= finalStats.feedback) {
        clearInterval(timer);
  ...
  return <section className="py-20 bg-gradient-to-br from-slate-50 to-gray-100 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-20 right-32 w-48 h-48 bg-gradient-to-r from-emerald-200 to-teal-200 rounded-full opacity-15 animate-bounce" style={{
        animationDelay: '2s'
      }}></div>
      </div>
        ...
        <AnimateOnScroll animation="slide-up" delay={400}>
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-gray-200">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {achievements.map((achievement, index) => <div key={achievement.title} className="text-center p-6 rounded-xl hover:bg-gray-50 transition-all duration-300 group animate-fade-in" style={{
